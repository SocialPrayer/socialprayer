            ...
            <div>
                ...
                <div class="prayalongdiv pull-right">
                	<span class="prayedalongcount small" :data-id="prayer.id">
                    	<!-- {{ (prayer.prayedalong.length > 0)  ? prayer.prayedalong.length + ' prayers': ''}} -->
                	</span>
                    <div class="btn-group">
...
<script>
    ...
    export default {
        ...
        methods: {
            listen(){
                window.Echo.channel('new-prayers')
                    .listen('NewPrayer', (data) => {
                        this.prayerlist.unshift(data.prayer);
                    });
                window.Echo.channel('prayalong')
                    .listen('PrayedAlong', (data) => {
                        console.log(data);
                        var _prayerid = data.prayerid;
                        $('.prayedalongcount[data-id="' + _prayerid + '"]').html(data.prayalongcount + ' prayers');
                    });
            },
            fetchData: function () {
